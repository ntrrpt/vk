...
def progress_upd(all, left, show):
    ...
    print(
        f"{show}: [{'#' * block + '-' * (7 - block)}] ({all} / {left}) {val_fix(progress * 100, 2)}%",
        end="\r",
    )
    ...
    while True:
        try:
            ...
            # idk
                ...
                logger.error("internal catched, waiting...   ")
                time.sleep(60)
...
def rqst_imgfile(i, img, _dir):
    ...
    def is_exist(filename, dir):  # what
        ...
        for file in dir:
            if (
                file[-3:] != "txt" and filename[len(str(progress_all)) + 1 : -4] in dir
            ):  # 999_ 'qwerty' .jpg
                return True
        return False
    ...
    if not is_exist(filename, _dir) or options.rewrite_files:
        ...
        if img["text"]:
            with open(f"{filename}_description.txt", "w", encoding="utf-8") as file:
                file.write(img["text"] + "\n")
...
def get_album(t_info, input_str, prefix):
    ...
    if "-" in owner_id and album is None:
        title = t_info[0]["name"]
    ...
    if count == 0:
        ...
        for i in range(offset_count):
            for t in reversed(range(1, options.requests_delay, 1)):
                print(
                    f"{progress_show}: ({offset_count} / {i + 1} / {len(photos['items'])} / {t})    ",
                    end="\r",
                )
                time.sleep(1)
    ...
    if album_id not in ["wall", "profile", "saved", "tagged"] and "error" not in album:
        ...
        created = datetime.datetime.fromtimestamp(
            album["items"][0]["created"]
        ).strftime("%Y-%m-%d %H:%M:%S")
        updated = datetime.datetime.fromtimestamp(
            album["items"][0]["updated"]
        ).strftime("%Y-%m-%d %H:%M:%S")
        description = (
            "id: %d\n"
            "thumb_id: %d\n"
        ...
        with open(f"{album_name}_description.txt", "w", encoding="utf-8") as file:
            file.write(description)

    # saving json with images
    hash_prefix = hashlib.md5(str(img_dict).encode("utf-8")).hexdigest()[:10]
    with open(f"{hash_prefix}_{album_name}_list.json", "w", encoding="utf-8") as file:
    ...
    if not options.simulate:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=options.threads_count
        ) as exector:
            future_to_img = {
                exector.submit(rqst_imgfile, k, img, _dir): (k, img)
    ...
    print(
        f"{progress_show}: [{datetime.timedelta(seconds=int(end_time))}] ({progress_all} / {progress_all})"
    )
    os.chdir("..")


def parse_link(
    i_work, num_seq=0, num_all=0, sw_steal=True
):  # input workstr, sequence str
    logger.info(f"in_link: {i_work}")
    # clean link
    if i_work[-6:] in ["?rev=1", "?rev=0"]:
        i_work = i_work[:-6]
    if i_work[:+8] == "https://":
        i_work = i_work[8:]
    if i_work[:+7] == "vk.com/":
    ...
    if sw_steal and "-" not in t_work:
        ...
        if options.steal_groups:
            groups = rqst_method("users.getSubscriptions", {"user_id": t_work})[
                "groups"
            ]
    ...
    if "-" in t_work:
        directory = (
            f"{str_cut(str_fix(check_group[0]['name']), 100)} (-{check_group[0]['id']})"
        )
    else:
    ...
    t_name_prefix = (
        f"[{num_seq} / {num_all} | " if num_all > 2 else "["
    ) + f"{str_cut(t_name)}]"

    # no access to albums
    if not albums:
        logger.error("no access to albums")
        return
    ...
    if albums is None:  # public detected
        ...
        if "-" in t_work:
            get_album(
                t_info,
                f"{t_work}_00",
                f"{t_name_prefix} 0 / {len(albums['items'])} (wall) | ",
            )
        ...
        for i, item in enumerate(albums["items"], start=1):
            if item["id"] == -6:
                get_album(
                    t_info,
                    f"{t_work}_0",
                    f"{t_name_prefix} {i} / {albums['count']} (profile) | ",
                )
                ...
                get_album(
                    t_info,
                    f"{t_work}_00",
                    f"{t_name_prefix} {i} / {albums['count']} (wall) | ",
                )
                ...
                get_album(
                    t_info,
                    f"{t_work}_000",
                    f"{t_name_prefix} {i} / {albums['count']} (saved) | ",
                )
                ...
                get_album(
                    t_info,
                    f"{t_work}_-9000",
                    f"{t_name_prefix} {i} / {albums['count']} (tagged) | ",
                )

            elif item["id"] == abs(item["id"]):
                get_album(
                    t_info,
                    f"{t_work}_{str(item['id'])}",
                    f"{t_name_prefix} {i} / {albums['count']} ",
                )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="login_info",
        default=login_info,
        help="Login info (token of login:pass)",
    )
    parser.add_option(
        "-t",
        "--threads",
    ...
    parser.add_option(
        ...
        dest="simulate",
        action="store_true",
        help="Simulate (not download, only json with urls)",
    )
    parser.add_option(
        "-d",
        "--delay",
        ...
        type=int,
        default="15",
        help="Delay between chunks requests (in seconds)",
    )
    parser.add_option(
        "-j", "--json", dest="conv_json", action="store_true", help="album.json parsing"
    )
    ...
    if options.conv_json:
        ...
        def rqst_json(filename):
            with open(filename) as file:
                ...
                end_time = val_fix(time.time() - start_time, 0)
                print(
                    f"{filename}: [{datetime.timedelta(seconds=int(end_time))}] ({progress_all} / {progress_all})"
                )
